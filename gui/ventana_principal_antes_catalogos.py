...
class VentanaPrincipal:
    ...
    def _actualizar_hojas(self, clave, hojas):
        if clave == "vida":
            self.checks_vida = llenar_checks(self.frame_vida_hojas, hojas)
            llenar_checks(self.frame_pc_vida_hojas, hojas)
        elif clave == "gmm":
            self.checks_gmm = llenar_checks(self.frame_gmm_hojas, hojas)
            llenar_checks(self.frame_pc_gmm_hojas, hojas)
        elif clave == "vlsp":
            self._llenar_combos([self.combo_vlsp, self.combo_vlsp_pc], hojas)
        elif clave == "tipo":
            self._llenar_combos([self.combo_tipo, self.combo_tipo_pc], hojas)
    ...
    def _hojas_seleccionadas(self, clave):
        frame = self.frame_pc_vida_hojas if clave == "vida" else self.frame_pc_gmm_hojas
        nombres = []
        for widget in frame.winfo_children():
            if isinstance(widget, tk.Checkbutton):
                variable = widget.cget("variable")
                if self.root.getvar(variable):
                    nombres.append(widget.cget("text"))
        return nombres
    ...
    def _worker_bases(self, mostrar_mensaje=True):
        try:
            generar_bases_sap(
                self.archivos["vida"],
                self.archivos["gmm"],
                self._hojas_seleccionadas("vida"),
                self._hojas_seleccionadas("gmm"),
                self.actualizar_estado,
            )
            if mostrar_mensaje:
                self.root.after(0, lambda: messagebox.showinfo("Proceso terminado", "Bases generadas correctamente."))
    ...
    def _worker_proceso_completo(self):
        ...
        try:
            self._worker_bases(mostrar_mensaje=False)
